...
import { 
  ...
  Fingerprint, ZapOff, Scale, HeartHandshake, Microscope
} from 'lucide-react';
...
const HUMAN_PROTOCOLS = [
  { id: 'authority', name: 'Authority Signature', icon: Fingerprint, desc: 'First-person veteran tone' },
  { id: 'burstiness', name: 'Rhythmic Burst', icon: ZapOff, desc: 'Sentence length variance' },
  { id: 'contrarian', name: 'Contrarian Expert', icon: Scale, desc: 'Challenges industry norms' },
  { id: 'empathy', name: 'Empathy Core', icon: HeartHandshake, desc: 'High psychological safety' },
  { id: 'analytical', name: 'Deep Analytics', icon: Microscope, desc: 'Cold, logic-first density' },
];
...
const App: React.FC = () => {
  ...
  const [inputs, setInputs] = useState<BlogInputs>({
    ...
    activeProtocols: ['authority', 'burstiness']
  });
  ...
  const toggleProtocol = (id: string) => {
    setInputs(prev => ({
      ...prev,
      activeProtocols: prev.activeProtocols.includes(id) 
        ? prev.activeProtocols.filter(p => p !== id) 
        : [...prev.activeProtocols, id]
    }));
  };
  ...
  const handleResearchKeywords = async () => {
    ...
    } catch (err) { setError("Research Protocol Timeout."); }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setChatMessages([{ role: 'assistant', content: "Neural narrative deployed. Humanity protocols active." }]);
    } catch (err: any) { setError(err.message || "Synthesis Protocol Fault."); }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [{ role: 'user', parts: [{ text: `Edit Request: ${userMsg}\nContext: ${currentBlog.htmlContent.substring(0, 3000)}` }] }],
        config: { systemInstruction: "Update the HTML payload. Maintain humanity score." }
      });
      const newHtml = response.text.trim().replace(/^```html\n?/i, '').replace(/\n?```$/i, '').trim();
      if (newHtml.length > 200) setCurrentBlog({ ...currentBlog, htmlContent: newHtml });
    } catch (err) { setError("Refinement Window Overflow."); }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#020305] flex flex-col font-jakarta transition-colors duration-500 overflow-x-hidden selection:bg-blue-500 selection:text-white">
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tighter uppercase flex items-center gap-2">
              ...
              <span className="hidden sm:inline-block px-3 py-1 rounded-full bg-blue-500/10 text-blue-500 text-[9px] tracking-widest font-black border border-blue-500/20 ml-2 uppercase">v17.5 ELITE</span>
            </h1>
            <p className="text-[9px] font-black opacity-30 uppercase tracking-[0.5em] leading-none mt-1.5">Human Protocol Layer • Neural SEO Engine</p>
          </div>
        </div>
        <button onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} className="w-14 h-14 bg-white dark:bg-slate-900 rounded-[1.25rem] border border-slate-200 dark:border-slate-700 flex items-center justify-center shadow-sm hover:rotate-6 transition-transform">
            ...
            <div className="bg-white dark:bg-slate-950 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-2xl p-10 space-y-8 sticky top-36">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input value={inputs.title} onChange={e => setInputs({...inputs, title: e.target.value})} placeholder="Primary keyword..." className="w-full bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl px-5 py-4 font-black outline-none focus:border-blue-500 transition-all text-sm" />
                </div>

                {/* Neural Protocols Grid */}
                <div className="pt-2">
                  <label className="text-[10px] font-black uppercase opacity-40 mb-4 block tracking-widest">Humanization Protocols</label>
                  <div className="grid grid-cols-1 gap-2.5">
                    {HUMAN_PROTOCOLS.map((protocol) => (
                      <button 
                        key={protocol.id} 
                        onClick={() => toggleProtocol(protocol.id)}
                        className={`group flex items-center gap-3.5 p-3.5 rounded-2xl border transition-all text-left ${inputs.activeProtocols.includes(protocol.id) ? 'bg-blue-600 border-blue-500 text-white shadow-xl shadow-blue-600/20' : 'bg-slate-50 dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-400 hover:border-blue-500/50'}`}
                      >
                        <protocol.icon className={`w-4 h-4 shrink-0 ${inputs.activeProtocols.includes(protocol.id) ? 'text-white' : 'text-blue-500'}`} />
                        <div>
                           <p className="text-[10px] font-black uppercase tracking-tight">{protocol.name}</p>
                           <p className={`text-[8px] font-bold opacity-60 leading-none mt-1 ${inputs.activeProtocols.includes(protocol.id) ? 'text-blue-100' : ''}`}>{protocol.desc}</p>
                        </div>
                        {inputs.activeProtocols.includes(protocol.id) && <Check className="w-3 h-3 ml-auto animate-in" />}
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <section className="lg:col-span-8 xl:col-span-6">
            {currentBlog ? (
              ...
              <div className="h-[1000px] flex flex-col items-center justify-center border-4 border-dashed border-slate-200 dark:border-slate-900 rounded-[5rem] opacity-30 group hover:opacity-50 transition-all">
                ...
                <p className="text-xl font-black uppercase tracking-[0.5em]">Neural Think Tank v17.5</p>
                <p className="text-[10px] font-black uppercase tracking-widest mt-4">Awaiting Blueprint Injection</p>
              </div>
            )}
          </section>
              ...
              <div className="bg-white dark:bg-slate-950 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-2xl p-8 sticky top-36 h-[calc(100vh-200px)] flex flex-col gap-10">
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-12">
                  <div className="pt-2 text-center">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.4em] opacity-40 mb-6">Semantic Integrity</h3>
                  ...
                  <div className="space-y-6">
                    <h4 className="text-[11px] font-black uppercase text-blue-600 flex items-center gap-2 px-1"><BarChart3 className="w-4 h-4" /> Realistic KD Analysis</h4>
                  ...
                  <div className="space-y-6 pb-4">
                    <h4 className="text-[11px] font-black uppercase text-blue-600 px-1">Evidence Sourcing</h4>
      ...
      <footer className="p-10 text-center border-t border-slate-200 dark:border-slate-800">
        <p className="text-[10px] font-black uppercase tracking-[0.5em] opacity-30">© 2025 Neural Architect • Zero-Cost SEO Elite Automation</p>
      </footer>
    </div>
  );
};
