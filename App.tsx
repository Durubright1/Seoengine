...
import { BlogInputs, GeneratedBlog, SearchIntent, ImageSource, GroundingSource, Country, SEOScoreResult, ChatMessage, KeywordMetric } from './types';
import { generateFullSuperPage, analyzeSEOContent } from './services/geminiService';
import { 
  Zap, Copy, Loader2, Globe, Moon, Sun,
  ExternalLink, Check, Sparkles, MessageSquare, 
  Send, Smartphone, BarChart3, Layout, ChevronUp, ChevronDown, 
  Trash2, Wand2, Target, Flame, HelpCircle, MapPin, ArrowRight
} from 'lucide-react';
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-xl p-8 sticky top-32">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input value={inputs.title} onChange={e => setInputs({...inputs, title: e.target.value})} placeholder="e.g. Best SEO Tools for 2025" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3.5 font-bold focus:ring-4 ring-blue-500/10 outline-none" />
                </div>
                <div>
                  <label className="text-[9px] font-black uppercase opacity-40 mb-2 block tracking-widest">Secondary Keywords (LSI)</label>
                  <input value={inputs.secondaryKeywords} onChange={e => setInputs({...inputs, secondaryKeywords: e.target.value})} placeholder="comma, separated, terms" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3.5 font-bold focus:ring-4 ring-blue-500/10 outline-none" />
                </div>
                ...
                <div>
                  ...
                  <textarea rows={6} value={inputs.customInstructions} onChange={e => setInputs({...inputs, customInstructions: e.target.value})} placeholder="Provide specific data, structure, or content requirements here..." className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3.5 font-bold focus:ring-4 ring-blue-500/10 outline-none resize-none" />
                </div>
              </div>
                ...
                <div className="xl:col-span-7 space-y-8">
                  <div className="bg-white dark:bg-[#0a0c10] rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden min-h-[900px] flex flex-col">
                    <div className="px-10 py-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-950/40">
                       ...
                       <button onClick={() => { navigator.clipboard.writeText(currentBlog.htmlContent); setCopied(true); setTimeout(() => setCopied(false), 2000); }} className="px-8 py-4 bg-slate-900 dark:bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase flex items-center gap-3 shadow-xl active:scale-95 transition-all">
                    ...
                    <div className="p-10 lg:p-14 flex-1 overflow-y-auto custom-scrollbar">
                      {previewMode === 'preview' ? <article className="blogger-preview animate-in fade-in" dangerouslySetInnerHTML={{ __html: currentBlog.htmlContent }} /> : <pre className="text-xs font-mono bg-slate-950 text-blue-400/80 p-10 rounded-3xl whitespace-pre-wrap leading-relaxed shadow-inner border border-slate-800 h-full">{currentBlog.htmlContent}</pre>}
                    </div>
                  </div>
                </div>

                {/* SEO Health & Strategist Sidebar */}
