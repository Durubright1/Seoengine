...
const LOADING_MESSAGES = [
  ...
  "Applying Burstiness Protocols...",
  "Sourcing HD Visual Assets...",
  "Grounding in 2024 Data...",
...
const KeywordAnalysisRow = ({ term }: { term: KeywordMetric }) => {
  const diffColor = term.difficulty > 70 ? 'bg-red-500' : term.difficulty > 40 ? 'bg-amber-500' : 'bg-emerald-500';
  return (
    <div className="flex flex-col gap-2 p-4 bg-slate-50/50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 group hover:border-blue-500/50 transition-all">
      <div className="flex justify-between items-center">
        <span className="text-xs font-black text-slate-700 dark:text-slate-300">{term.keyword}</span>
        <span className="text-[10px] font-bold opacity-60">Vol: {(term.volume/1000).toFixed(1)}k</span>
      </div>
      <div className="w-full h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
        <div className={`h-full ${diffColor} transition-all duration-1000`} style={{ width: `${term.difficulty}%` }} />
      </div>
...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setChatMessages([{ role: 'assistant', content: "Think Tank synthesis complete. Humanity Protocols at 99.2% efficiency." }]);
    } catch (err: any) { setError(err.message || "Protocol Fault."); }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [{ role: 'user', parts: [{ text: `Request: ${userMsg}\nContext: ${currentBlog.htmlContent.substring(0, 4000)}` }] }],
        config: { systemInstruction: "Update the HTML based on request. Maintain high humanity and SEO standards." }
      });
      const newHtml = response.text.trim().replace(/^```html\n?/i, '').replace(/\n?```$/i, '').trim();
      if (newHtml.length > 500) setCurrentBlog({ ...currentBlog, htmlContent: newHtml });
    } catch (err) { setError("Chat context full."); }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#020305] flex flex-col font-jakarta transition-colors duration-500 overflow-x-hidden selection:bg-blue-500 selection:text-white">
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tighter uppercase flex items-center gap-2">
              SuperPage <span className="text-blue-600 italic">ThinkTank</span>
              <span className="hidden sm:inline-block px-3 py-1 rounded-full bg-blue-500/10 text-blue-500 text-[9px] tracking-widest font-black border border-blue-500/20 ml-2 uppercase">ELITE v16.0</span>
            </h1>
            <p className="text-[9px] font-black opacity-30 uppercase tracking-[0.5em] leading-none mt-1.5">3,000-Word Human Protocol • Deep Grounding</p>
          </div>
        </div>
        <button onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} className="w-14 h-14 bg-white dark:bg-slate-900 rounded-[1.25rem] border border-slate-200 dark:border-slate-700 flex items-center justify-center shadow-sm hover:scale-105 transition-transform">
      ...
      <main className="flex-1 max-w-[1900px] mx-auto w-full p-8 lg:p-12">
        <div className="grid lg:grid-cols-12 gap-12">
          ...
          <aside className="lg:col-span-4 xl:col-span-3 space-y-8">
            <div className="bg-white dark:bg-slate-950 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-2xl p-10 space-y-8">
              <h2 className="text-[11px] font-black uppercase tracking-[0.3em] text-blue-600 flex items-center gap-3"><Target className="w-5 h-5" /> Narrative Blueprint</h2>
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-black uppercase opacity-40 mb-3 block tracking-widest">Region</label>
                ...
                <div>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <button onClick={handleResearchKeywords} disabled={researching} className="text-[9px] font-black text-blue-600 hover:opacity-70 transition-opacity uppercase flex items-center gap-1">
                      {researching ? <Loader2 className="w-3 h-3 animate-spin" /> : <Wand2 className="w-3 h-3" />} Research
                    </button>
                  </div>
                  <textarea value={inputs.secondaryKeywords} onChange={e => setInputs({...inputs, secondaryKeywords: e.target.value})} placeholder="Keywords..." className="w-full bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl px-5 py-4 font-black text-xs h-28 resize-none outline-none focus:border-blue-500" />
                </div>
              </div>
              <button onClick={handleGenerate} disabled={loading} className="w-full py-6 bg-blue-600 hover:bg-blue-700 text-white rounded-[2rem] font-black text-xs tracking-[0.3em] flex flex-col items-center transition-all disabled:opacity-50 shadow-2xl">
                {loading ? <><Loader2 className="animate-spin w-6 h-6 mb-2" /><span className="text-[9px] uppercase animate-pulse">{loadingStep}</span></> : <><Sparkles className="w-5 h-5 mr-2 mb-1" /> SYNC THINK TANK</>}
              </button>
            </div>
          </aside>
          ...
          <section className="lg:col-span-8 xl:col-span-9">
            {currentBlog ? (
              <div className="grid xl:grid-cols-12 gap-10">
                <div className="xl:col-span-8 space-y-10">
                  <div className="grid sm:grid-cols-3 gap-6">
                    <MetricCard icon={UserCheck} label="Humanity Score" value={`${currentBlog.seoResult?.humanityScore}%`} colorClass="bg-emerald-500" />
                    <MetricCard icon={Activity} label="Burstiness" value={`${currentBlog.seoResult?.burstinessIndex}/100`} colorClass="bg-blue-500" />
                    <MetricCard icon={Layers} label="Authority" value={`${currentBlog.seoResult?.authoritySignal}%`} colorClass="bg-purple-500" />
                  </div>

                  <div className="bg-white dark:bg-slate-950 rounded-[4rem] border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden min-h-[900px] flex flex-col relative group">
                    {isCopying && (
                      <div className="absolute inset-0 z-[50] flex items-center justify-center pointer-events-none">
                        <div className="absolute inset-0 bg-blue-600/10 animate-pulse" />
                        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-2xl border border-blue-500/50 flex items-center gap-4 animate-bounce">
                           <Check className="text-emerald-500 w-6 h-6" />
                           <span className="text-xs font-black uppercase tracking-widest">Neural Assets Copied</span>
                        </div>
                      </div>
                    )}
                    <div className="px-10 py-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
                       <div className="flex bg-white dark:bg-slate-900 rounded-2xl p-1.5 border border-slate-200 dark:border-slate-800 shadow-inner">
                          <button onClick={() => setPreviewMode('preview')} className={`px-8 py-3.5 rounded-xl text-[10px] font-black uppercase transition-all ${previewMode === 'preview' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400'}`}>Visual</button>
                          <button onClick={() => setPreviewMode('html')} className={`px-8 py-3.5 rounded-xl text-[10px] font-black uppercase transition-all ${previewMode === 'html' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400'}`}>HTML</button>
                       </div>
                       <button onClick={handleCopy} className={`px-10 py-5 rounded-2xl text-[10px] font-black uppercase flex items-center gap-4 transition-all hover:scale-105 ${isCopying ? 'bg-emerald-500' : 'bg-slate-900 dark:bg-blue-600'} text-white shadow-2xl`}>
                          {isCopying ? <Check className="w-5 h-5" /> : <Copy className="w-5 h-5" />} {isCopying ? 'EXTRACTED' : 'EXTRACT PAYLOAD'}
                       </button>
                    </div>
                    <div className={`p-10 lg:p-20 flex-1 overflow-y-auto custom-scrollbar transition-opacity ${isCopying ? 'opacity-30 blur-sm' : 'opacity-100'}`}>
                      {previewMode === 'preview' ? <article className="blogger-preview animate-in" dangerouslySetInnerHTML={{ __html: currentBlog.htmlContent }} /> : <pre className="text-[11px] font-mono bg-slate-950 text-emerald-400/80 p-12 rounded-[2.5rem] whitespace-pre-wrap leading-relaxed border border-slate-800 h-full">{currentBlog.htmlContent}</pre>}
                    </div>
                  </div>
                </div>

                <aside className="xl:col-span-4 space-y-10">
                  <div className="bg-white dark:bg-slate-950 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-2xl p-10 sticky top-36 h-[calc(100vh-200px)] flex flex-col">
                    <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-12">
                      <div className="text-center pt-2">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.4em] opacity-40 mb-6">Realistic SEO Audit</h3>
                      ...
                      <div className="space-y-6">
                        <h4 className="text-[11px] font-black uppercase text-blue-600 flex items-center gap-2"><BarChart3 className="w-4 h-4" /> Realistic Keyword KD</h4>
                        <div className="space-y-4">
                          {currentBlog.seoResult?.terms.map((term, i) => (
                            <KeywordAnalysisRow key={i} term={term} />
                          ))}
                        </div>
                      </div>

                      <div className="py-8 border-y border-slate-100 dark:border-slate-800 space-y-6">
                        <h4 className="text-[11px] font-black uppercase opacity-40 flex items-center gap-2"><MessageSquare className="w-4 h-4 text-blue-600" /> Narrative Refiner</h4>
                        ...
                        <div className="relative group">
                          <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSendMessage()} placeholder="Request edits..." className="w-full bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl px-5 py-4 text-xs font-bold pr-12 outline-none focus:border-blue-500 transition-all shadow-inner" />
                      ...
                      <div className="space-y-6">
                        <h4 className="text-[11px] font-black uppercase text-blue-600">Verified Citations</h4>
              ...
              <div className="h-[850px] flex flex-col items-center justify-center border-4 border-dashed border-slate-200 dark:border-slate-900 rounded-[5rem] opacity-30 group hover:opacity-50 transition-all cursor-default">
                <BrainCircuit className="w-32 h-32 mb-8 group-hover:scale-110 group-hover:rotate-6 transition-transform text-blue-600" />
                <p className="text-xl font-black uppercase tracking-[0.5em]">Neural Think Tank v16.0 Ready</p>
                <p className="text-[10px] font-black uppercase tracking-widest mt-4">Awaiting Focus Keyword Payload</p>
              </div>
            )}
          </section>
        </div>
      ...
      <footer className="p-10 text-center border-t border-slate-200 dark:border-slate-800">
        <p className="text-[10px] font-black uppercase tracking-[0.5em] opacity-30">© 2025 Neural Architect • High-Fidelity SEO Environment</p>
      </footer>
    </div>
  );
};
